...
#Creating DB & Table:--->
try:
    con = connect(host = 'localhost', user = 'root', password = 'GUPTA10@MySQL')
    csr = con.cursor()
...
def play():
    ...
    print('Tail     -           1')
    while not SC in [0, 1]:
        SC = int(input('Select your choice(0/1): '))
    TS = randint(0, 1)
    if SC==TS:
        TW = 1
        print('\n\n\n\n\nYOU WON THE TOSS !!!\n\n\n\n')
...
while True:
    if login():
        try:
            con = connect(host = 'localhost', user = 'root', password = 'GUPTA10@MySQL', database = 'Users')
            csr = con.cursor()
